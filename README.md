...
To view administrator-specific options (Admin Dashboard, global history, user management,add category and sub category),
you must log in using the Admin user created during the setup process.

The admin role cannot be selected during registration.
Admin privileges are granted only to the predefined Admin user generated by running:
npm run setup(So that not everyone can claim to be an administrator when entering the site,His details are listed above - enter the details and you will be able to see the administrator's functionality)
